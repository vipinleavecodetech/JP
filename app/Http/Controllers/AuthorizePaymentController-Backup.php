...
class AuthorizePaymentController extends Controller
{
    ...
    public function processSubscriptionAuthorised(Request $request)
    {
        ...
        $merchantAuthentication->setTransactionKey('3st244m3N285CHtE');
        ////
        Patient::where('patient_id', $request->patient_id)->update(['sure_script_patient_id' => $request->sure_script_patient_id]);
        ...
        if (($response != null) && ($response->getMessages()->getResultCode() == "Ok") )
        {
            ...
            $admin_content = "Hello Admin,<br><br>New customer is registerd with HEADS UP.<br><br>
            ...
            // Mail::send([], $transaction->toArray(), function ($message) use ($user_content, $admin_content, $customer_mail, $admin_mail) {
            // $message
            // ->to($customer_mail)
