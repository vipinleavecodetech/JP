    ...
    <div class="content">
       <div class="container-fluid">
                ...
                <div class="page-title-box" style="display:block">
                   <div class="page-title-right">
                      <ol class="breadcrumb m-0">
                         <li class="breadcrumb-item"><a href="{{ url('/') }}" class="logo-dark">
                                      <span class="logo-sm">
          ...
          <!-- end page title --> 
                ...
                <div class="card-box">
                    <select name="prescription_template">
                        <option value="">Select Prescription Template</option>
                        @foreach($PrescriptionTemplate as $template)
                        <option value="{{$template->id}}">{{$template->name}}</option>
                        @endforeach
                    </select>
                    <form action="{{ route('admin.patients.prescriptionUpdate', $patient->id) }}" method="post">
                        @csrf
                        <textarea  id="editor1" name="prescription_area" placeholder="Write your Prescription Here...." style="display:none"></textarea>
                        <input type="hidden" name="patient_id" value="{{$patient->patient_id}}">
                        <div class="col-12">
                           <input type="submit" name="submit" class="btn btn-success next action-button" value="Update Prescription" style="text-align: center; margin-top: 50px;">
                        </div>
                     </form>
                     <?php } ?>
                    ...
                    <div class="container-fluid cust_pres">
                        <div class="row">
                            <div class="col-lg-12">
                                ...
                                <?php //echo "<pre>"; print_r($prescription_detail);?>
...
<script>
    $(document).ready(function () {
        // Declare a variable to store the CKEditor instance
        var ckEditorInstance;
        $('select[name="prescription_template"]').on('change', function () {
            var templateId = $(this).val();
            if(templateId) {
                $.ajax({
                    url: '/getPrescriptionData/' + templateId+ '?customerID={{$patient->patient_id}}',
                    type: 'GET',
                    dataType: 'json',
                    success: function (data) {
                        ...
                        // Set the content and create a new CKEditor instance
                        $("#editor1").val(data);
                        ckEditorInstance = CKEDITOR.replace('editor1', {
                            height: '250px',
                            width: '100%',
